...
description: "Active disease-outbreak alerts worldwide, severity-ranked and searchable — aggregated from WHO DON, CDC HAN, Outbreak News Today, and ProMED."
---
...
- **Search** — free-text match against country / region / disease (search field above the pills). Lowercased substring match; severity filter and search can be combined.
- **Source link in-row** — each row renders the upstream source name as an inline link (sanitised URL, `target="_blank"`). Clicking the source-name link opens the upstream advisory.
- **Row → map** — click a row (outside the source link) to center the map on the outbreak coords or country centroid and enable the disease layer when needed. Location line falls back to `countryCode` when title extraction left `location` empty.
