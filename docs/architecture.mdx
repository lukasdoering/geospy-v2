...
**Analysis of Competing Hypotheses (ACH)** — composite intelligence products reduce single-source bias by corroborating independent inputs before they move a score: the Country Instability Index blends unrest, conflict, security, and information-velocity signals; protest data merges ACLED and GDELT; disaster data fuses USGS, GDACS, and NASA EONET. The breaking-news alert pipeline is deliberately different — its five origins (RSS alert, keyword spike, hotspot escalation, military surge, OREF siren) are *independent* producers, any of which can raise a critical alert on its own. Rather than requiring cross-stream convergence, that pipeline suppresses single-source noise with layered gates — per-event dedup, a global cooldown, a recency window, source-tier LLM confirmation for Tier 3+ outlets, story-phase filtering, and an importance floor — while ground-truth sources such as OREF sirens escalate directly.
