...
export class EnergyRiskOverviewPanel extends Panel {
  ...
  private onEuGasFocus: (() => void) | null = null;
  private onDisruptionsFocus: (() => void) | null = null;
  ...
  constructor() {
    ...
    this.content.addEventListener('click', (e) => {
      const tile = (e.target as HTMLElement).closest('.ero-tile-action') as HTMLElement | null;
      if (!tile) return;
      const action = tile.dataset.eroAction;
      if (action === 'hormuz') this.onHormuzFocus?.();
      else if (action === 'eu-gas') this.onEuGasFocus?.();
      else if (action === 'disruptions') this.onDisruptionsFocus?.();
    });
  ...
  public setEuGasFocusHandler(handler: () => void): void {
    this.onEuGasFocus = handler;
  }

  public setDisruptionsFocusHandler(handler: () => void): void {
    this.onDisruptionsFocus = handler;
  }
...
const RISK_OVERVIEW_CSS = `
  ...
  .ero-tile-action:hover,
  .ero-tile-hormuz:hover {
    filter: brightness(1.08);
  }
  .ero-tile-action:focus-visible,
